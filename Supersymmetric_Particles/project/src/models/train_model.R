...
X <- train_data[, !(names(train_data) %in% "Signal")]
y <- train_data$Signal
...
lasso_model <- cv.glmnet(train_X, train_y, alpha = 1,nfolds = 5) # alpha = 1 for Lasso

# Train Ridge regression model
ridge_model <- cv.glmnet(train_X, train_y, alpha = 0,nfolds = 5 ) # alpha = 0 for Ridge
